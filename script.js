// ============================================================
//  Palak Thakur — Portfolio  |  script.js
//  Features:
//    • Admin password-protected gallery uploads
//    • localStorage persistence (photos survive refresh)
//    • Crop modal, multi-photo slides, delete
//    • Gamification HUD, sparkles, cursor, theme toggle
// ============================================================

// ─────────────────────────────────────────────────────────────
//  ADMIN GALLERY PASSWORD
//  Change 'palak2007' to whatever you want.
//  Set adminUnlocked = true in console to bypass during dev.
// ─────────────────────────────────────────────────────────────
const ADMIN_PASS   = 'palak2007';
let   adminUnlocked = false;

function checkAdmin() {
  if (adminUnlocked) return true;
  const pass = prompt('🔐 Admin password required to upload photos:');
  if (pass === null) return false;           // user cancelled
  if (pass === ADMIN_PASS) {
    adminUnlocked = true;
    toast('🔓 Admin mode unlocked — upload away!');
    return true;
  }
...
// ─────────────────────────────────────────────────────────────
//  LOCALSTORAGE — persist gallery photos across page reloads
// ─────────────────────────────────────────────────────────────
const LS_KEY = 'pt-gallery-v1';
...
function saveGalleryData(data) {
  ...
  catch { toast('⚠️ Storage full — old photos may be lost.'); }
}

const galleryStorage = loadGalleryData();   // { "0": [...base64], "1": [...], … }
...
const PROJ = [
  ...
  },
  {
    ...
    title : "CodeHer Elite",
    desc  : "A hackathon project focused on deepfake detection. The platform helps users identify potentially manipulated media using AI-based analysis.",
  ...
  },
  {
    ...
    desc  : "A responsive portfolio website showcasing my projects, technical skills, achievements, and learning journey as an EEE student interested in software development.",
    stack : ["HTML","CSS","JavaScript"],
    gh    : "https://github.com/palakthakur07"
  }
];
...
EXP.forEach((e, i) => {
  ...
  ni.innerHTML = `
    ...
    <div>
      <div class="exp-nav-year">${e.year}</div>
      <div class="exp-nav-title">${e.title}</div>
    </div>`;
...
let cropCallback = null, cropImgNatW = 0, cropImgNatH = 0;
let cropX = 0, cropY = 0, cropScale = 1;
let cropDragging = false, cropStartX = 0, cropStartY = 0, cropStartTX = 0, cropStartTY = 0;
...
function openCrop(src, cb) {
  ...
  cropImg.src = src;
  cropImg.onload = () => {
    cropImgNatW = cropImg.naturalWidth;
    cropImgNatH = cropImg.naturalHeight;
...
cropModal.querySelector('#cropConfirm').addEventListener('click', () => {
  ...
  ctx.scale(2, 2);
  ctx.drawImage(cropImg, (cropX) / cropScale, (cropY) / cropScale, areaW / cropScale, areaH / cropScale, 0, 0, areaW, areaH);
...
GALLERY_ITEMS.forEach((item, i) => {
  /* Photos for this tile — restored from localStorage */
  const tilePhotos = galleryStorage[i] ? [...galleryStorage[i]] : [];
  let currentIdx  = 0;

  const div = document.createElement('div');
  div.className = 'g-item';
  div.style.cssText = (item.span ? 'grid-column:span 2;grid-row:span 2;' : '') +
    'position:relative;overflow:hidden;cursor:default;border-radius:var(--radius)';
  ...
  div.innerHTML = `
    ...
    <label for="gfile${i}" style="position:absolute;bottom:.5rem;right:.5rem;width:28px;height:28px;border-radius:50%;background:var(--pink);display:flex;align-items:center;justify-content:center;cursor:pointer;transition:.2s;border:2px solid #fff;z-index:10" title="Add photo (admin only)">
  ...
  function renderSlides() {
    ...
    tilePhotos.forEach((src, j) => {
      ...
      img.src = src;
      img.style.cssText = 'width:100%;height:100%;object-fit:cover;position:absolute;inset:0;opacity:0;transition:opacity .35s';
    ...
    delBtn.style.display  = adminUnlocked ? 'flex' : 'none';   // only show delete to admin
  }
  ...
  delBtn.addEventListener('click', e => {
    ...
    // Persist deletion
    galleryStorage[i] = [...tilePhotos];
    saveGalleryData(galleryStorage);
    renderSlides();
    toast('🗑 Photo removed');
  });

  /* ── + button — admin password gate ────────────────────── */
  const label = div.querySelector(`label[for="gfile${i}"]`);
  label.addEventListener('click', e => {
    if (!checkAdmin()) {
      e.preventDefault();
      return false;
    }
    // If admin unlocked, show delete buttons on existing slides
    if (tilePhotos.length > 0) delBtn.style.display = 'flex';
  });
  ...
  if (tilePhotos.length > 0) renderSlides();
});
...
// ─────────────────────────────────────────────────────────────
const mascots = ['fa-user','fa-star','fa-heart','fa-bolt','fa-brain','fa-code','fa-rocket','fa-graduation-cap'];
let mascotIdx = 0;
document.getElementById('heroMascot').addEventListener('click', e => {
  mascotIdx = (mascotIdx + 1) % mascots.length;
  document.querySelector('.hero-img-emoji').innerHTML =
    `<i class="fas ${mascots[mascotIdx]}" style="font-size:5rem;color:var(--pink);opacity:.35"></i>`;
...
const BADGE_MSGS = [
  "🔍 Explorer — you started the journey!",
  "📖 Reader — you know who I am!",
  "🔬 Researcher — you studied my path!",
  "🛠️ Builder — you checked my work!",
  "🎨 Creative — you saw my gallery!",
  "🏆 Champion — you explored everything!"
];
let earned = [];
...
function earnBadge(id, msg) {
  ...
  document.querySelectorAll('.hud-star').forEach((s, i) => { if (i < earned.length) s.classList.add('on'); });
